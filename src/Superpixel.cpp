...
/*Destrutor, para apagar os dados presentes*/
Slic::~Slic()
{
    clear_data();
}

/*Apaga os dados salvos pelo algoritmo*/
...
cv::Point Slic::find_local_minimum(const cv::Mat &image, cv::Point center)
{
    ...
    for (int i = center.x-1; i < center.x+2; i++)
	{
		...
		{
            cv::Vec3b clr = image.at<cv::Vec3b>(j+1, i);
            cv::Scalar c1(clr[0],clr[1],clr[2]);
            clr = image.at<cv::Vec3b>(j, i+1);
            cv::Scalar c2(clr[0],clr[1],clr[2]);
            clr = image.at<cv::Vec3b>(j, i);
...
{
    std::vector<cv::Vec3b> colours(centers.size());
